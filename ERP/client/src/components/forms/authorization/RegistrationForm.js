...
const SignUp = () => {
    ...
    const handleRegister = async (e) => {
        e.preventDefault();
        const { employeeSiteId, fName, lName, mName, password, userRoleId, position } =
            value;
        if (!fName) {
            ...
            toast.error("Please provide your employee site id", toastOption);
        } else if (!userRoleId) {
            toast.error("Please provide your User Role Id", toastOption);
        } else if (password !== passwordConfirm) {
            toast.error("Your password and confirm password is not match", toastOption);
        } else if (password.length < 8) {
            toast.error("The password must be greaterthan 8 characters", toastOption);
        } else {
            try {
                    ...
                    <div>
                        <div className="form-group">
                            ...
                            <input
                                name="mName"
                                onChange={(e) =>
                                    setValue({ ...value, mName: e.target.value })
                                }
                                value={value.mName}
                                type="text"
                                className="form-control"
