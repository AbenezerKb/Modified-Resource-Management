...
import { ITEMTYPE, PURCHASEREPORTGROUPBY, PURCHASESTATUS } from "../../../Constants";
import { fetchEquipmentCategories, fetchEquipmentCategory } from "../../../api/category";
import { fetchPurchaseReport } from "../../../api/report";
import { fetchEmployees } from "../../../api/employee";
import PurchaseStatusBadge from "../purchase/fragments/PurchaseStatusBadge";
...
function initFormValues() {
    return {
        ...
        employeeRole: "-1",
        employeeId: "-1",
        groupBy: PURCHASEREPORTGROUPBY.STATUS.key,
    };
}
...
function PurchaseReport() {
    ...
    const {
        ...
        error,
        refetch,
    } = useQuery(["purchase", formValues], () => fetchPurchaseReport(formValues), {
        enabled: false,
    });

    function valueChanged(e) {
        queryClient.removeQueries(["purchase", formValues]);
    ...
    const setChartDataCallback = useCallback(
        (summary) => {
            ...
            summary = summary.map((current) => {
                ...
                if (groupBy === PURCHASEREPORTGROUPBY.STATUS.key)
                    label = PURCHASESTATUS[current.key];
                else if (
                    groupBy === PURCHASEREPORTGROUPBY.REQUESTEDBY.key ||
                    groupBy === PURCHASEREPORTGROUPBY.APPROVEDBY.key ||
                    groupBy === PURCHASEREPORTGROUPBY.PURCHASEDBY.key ||
                    groupBy === PURCHASEREPORTGROUPBY.CHECKEDBY.key
                )
                    label = current.value ? `${current.value.fName} ${current.value.mName}` : "N/A";
                else if (groupBy === PURCHASEREPORTGROUPBY.SITE.key) label = current.value.name;
    ...
    useEffect(() => {
        ...
        Object.keys(data).map((key, index) => {
            ...
            data[key].checkDate =
                data[key].checkDate && new Date(data[key].checkDate).toLocaleDateString();

            //format names
            data[key].requestedByName =
                data[key].requestedBy &&
                `${data[key].requestedBy.fName} ${data[key].requestedBy.mName}`;
            ...
            data[key].checkedByName =
                data[key].checkedBy && `${data[key].checkedBy.fName} ${data[key].checkedBy.mName}`;
            ...
            data[key].open = (
                <>
                    <Link className="btn btn-teal me-2" to={`/purchase/${data[key].purchaseId}`}>
                    ...
                    <FormRow
                        ...
                        valueL={
                            <Form.Select
                                ...
                                onChange={valueChanged}
                            >
                                {Object.keys(PURCHASEREPORTGROUPBY).map((i) => (
                                    <option key={i} value={PURCHASEREPORTGROUPBY[i].key}>
                                        {PURCHASEREPORTGROUPBY[i].label}
                                    </option>
                                ))}
                            </Form.Select>
                        }
                ...
                {chartData && (
                    <>
                        ...
                        <Container fluid className="px-4">
                            <h1 className="text-center mb-2 fs-4">Purchase Report Summary</h1>
                                        ...
                                        <th>
                                            {
                                                PURCHASEREPORTGROUPBY[
                                                    Object.keys(PURCHASEREPORTGROUPBY).find(
                                                        (i) =>
                                                            PURCHASEREPORTGROUPBY[i].key ===
                                                            formValues.groupBy
                                                    )
                                                ]?.label
                                            }
